...
const char binHeader[] =
    "{\"01\":{\"name\":\"count\",\"size\":8,\"scale\":1,\"signed\":0},\
...
void TerminalCommands::PrintParamsJson(Terminal* term, char* arg)
{
    ...
    for (uint32_t idx = 0; idx < Param::PARAM_LAST; idx++)
    {
        ...
        {
            // fprintf(term, "%c\r\n   \"%s\":
            // {\"unit\":\"%s\",\"value\":%f,",comma, pAtr->name, pAtr->unit,
            // Param::Get((Param::PARAM_NUM)idx));
            str.asprintf(
                "%c\r\n   \"%s\": {\"unit\":\"%s\",\"value\":%.2f,",
                comma,
                pAtr->name,
                pAtr->unit,
                Param::GetFloat((Param::PARAM_NUM)idx));
            out << str;
            ...
            if (Param::IsParam((Param::PARAM_NUM)idx))
            {
                ...
                str.asprintf(
                    "\"isparam\":true,\"minimum\":%.2f,\"maximum\":%.2f,"
                    "\"default\":%.2f,\"category\":\"%s\",\"i\":%d}",
                    FP_TOFLOAT(pAtr->min),
                    FP_TOFLOAT(pAtr->max),
                    FP_TOFLOAT(pAtr->def),
                    pAtr->category,
